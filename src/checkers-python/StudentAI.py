    ...
    def get_move(self, move):
        if len(move) != 0:  # if opponent goes first, then our color stays 2 (white)
            if self.startnode.children:
                ...
                newnode = self.create_node_child(self.startnode, self.opponent[self.color],
                                                 move)  # update board with opp move
        ...
        # create a copy of the startnode so we can only make changes to the copy
        self.startnode.color = self.color
